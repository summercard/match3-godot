...
func _run() -> void:
	...
	_assert((scene.get_node("ProductGrid/Cards/Card1/Frame") as TextureRect).texture.resource_path.ends_with("shop/concept/image2/ui_shop_card_panel_image2_clean.png"), "product card art should come from image-2 transparent asset")
	...
	_assert((scene.get_node("BottomNav/Frame") as TextureRect).texture.resource_path.ends_with("main/lobby_refresh/ui_bottom_nav_panel_v3.png"), "shop bottom nav should reuse main lobby art")
