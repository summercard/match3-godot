...
func _run() -> void:
	...
	_expect(int(three_star.get("def", 0)) > int(MonsterDb.get_monster("monster_006").get("baseDEF", 0)), "high-level DEF should use the current flat growth rule")
	_expect(int(legacy_three_star.get("def", 0)) == int(three_star.get("def", 0)), "legacy UI stats should match battle DEF")
	_expect(DamageCalculatorScript.get_defense_reduction(float(three_star.get("def", 0))) > 0.0, "current three-star DEF should reduce incoming damage")
