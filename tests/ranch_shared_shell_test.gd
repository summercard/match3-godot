...
func _run() -> void:
	...
	_expect((ranch.get_node("Background") as TextureRect).texture.resource_path.ends_with("bg_pet_academy.png"), "classroom should switch to academy background")
	...
	_expect((ranch.get_node("Background") as TextureRect).texture.resource_path.ends_with("bg_social_meadow_yard.png"), "social should start with clean lodge background")
	...
	_expect((ranch.get_node("Background") as TextureRect).texture.resource_path.ends_with("bg_social_sunny_yard.png"), "social place cycling should replace the scene background")
	(ranch.get_node("PetFarmBottomNav/Nav2") as Button).pressed.emit()
