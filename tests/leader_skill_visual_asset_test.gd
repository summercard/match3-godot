...
func _run() -> void:
	...
	if scene != null:
		...
		_expect(float(showcase.get("banner", 0.0)) <= 0.5, "leader burst skill name banner should be shortened by 0.5s")
		...
		_expect(str(showcase.get("playback_mode", "")) == "non_blocking_combo", "leader burst playback should not block combo resolution")
		_expect(not bool(showcase.get("dark_overlay_enabled", true)), "leader burst showcase should not draw a dark full-screen overlay")
		_expect(float(showcase.get("scale_out", 0.0)) >= 0.5, "leader burst portrait scale-out should take 0.5s")
		_expect(float(showcase.get("effect_hold", 0.0)) <= 1.5, "leader burst effects should use the shortened hold time")
		_expect(float(showcase.get("restore", 0.0)) <= 0.02, "leader burst dark overlay restore should be effectively removed")
		scene.queue_free()
