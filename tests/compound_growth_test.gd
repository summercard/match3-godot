...
func _run() -> void:
	var expected_rates := {1: 0.065, 2: 0.091, 3: 0.104, 4: 0.156, 5: 0.195}
	...
	for stat_key in ["hp", "atk", "spd"]:
		var expected_level_2 := int(float(level_1.get(stat_key, 0)) * 1.065)
		_assert(int(level_2.get(stat_key, 0)) == expected_level_2, "one-star %s should grow six and a half percent from level 1 to level 2" % stat_key)
	_assert(int(level_2.get("def", 0)) == int(level_1.get("def", 0)), "DEF should use small flat increments instead of percentage growth")
	var expected_level_20_hp := int(float(level_1.get("hp", 0)) * pow(1.065, 19.0))
	_assert(int(level_20.get("hp", 0)) == expected_level_20_hp, "level 20 HP should use compounded growth")
	_assert(float(level_20.get("hp", 0)) / float(level_1.get("hp", 1)) > 3.0, "one-star level 20 should exceed three times its level 1 HP")
