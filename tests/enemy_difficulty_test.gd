...
func _test_enemy_boss_multiplier_is_runtime_only() -> void:
	...
	_assert(is_equal_approx(float(multiplier.get("hp", 0.0)), 7.0), "enemy boss HP multiplier should use the clean boss pressure value")
	_assert(is_equal_approx(float(multiplier.get("atk", 0.0)), 1.5), "enemy boss ATK multiplier should use the clean boss pressure value")
	...
	_assert(int(enemy_boss.get("hp", 0)) >= int(owned_boss.get("hp", 0)) * 6, "enemy boss HP should gain boss pressure after level growth")
