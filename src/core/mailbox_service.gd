...
func create_tower_reward_mail(season_id: String, floor: int, reward: Dictionary) -> Dictionary:
	...
	var mail := {
		...
		"source": "tower_reward",
		"sender_name": "共鸣塔补给站",
		"title": TranslationServer.translate("第 %d 层阶段补给") % floor,
		"body": TranslationServer.translate("你已突破共鸣塔第 %d 层，阶段补给已寄入信箱。") % floor,
...
func create_tower_consolation_mail(season_id: String, checkpoint_floor: int, reward: Dictionary) -> Dictionary:
	...
	var mail := {
		...
		"sender_name": "旅行精灵驿站",
		"title": TranslationServer.translate("第 %d 层远征鼓励") % checkpoint_floor,
		"body": TranslationServer.translate("这次远征先在第 %d 层休整吧。旅行精灵送来一份轻松补给，整理好队伍再来挑战！") % checkpoint_floor,
		"attachments": attachments,
		"created_at": Time.get_unix_time_from_system(),
		"read_at": null,
		"claimed_at": null,
